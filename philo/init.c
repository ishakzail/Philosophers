...
/*   Updated: 2022/05/25 21:39:07 by izail            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_info(t_info info, int ac, char **av)
{
	...
	if (info.number_of_philosophers < 0 || !check_if_int(av[1]))
		return (ft_puterr("ERROR : Wrong number of philosophers"));
	if (info.time_to_die < 60 || !check_if_int(av[2]))
		return (ft_puterr("ERROR : Wrong number of time_to_die\n"));
	if (info.time_to_eat < 60 || !check_if_int(av[3]))
		return (ft_puterr("ERROR : Wrong number of time_to_eat\n"));
	if (info.time_to_sleep < 60 || !check_if_int(av[4]))
